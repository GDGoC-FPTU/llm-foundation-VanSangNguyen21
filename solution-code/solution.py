...
def compare_models(prompt: str) -> dict:
    ...
    # Calculate costs using PRICING_1M_TOKENS
    def calculate_cost(model: str, input_tokens: int, output_tokens: int) -> float:
        pricing = PRICING_1M_TOKENS[model]
        cost = (input_tokens * pricing["input"] + output_tokens * pricing["output"]) / 1_000_000
        return cost
    
    gpt4o_cost = calculate_cost(OPENAI_MODEL, gpt4o_usage["input_tokens"], gpt4o_usage["output_tokens"])
    gpt4o_mini_cost = calculate_cost(OPENAI_MINI_MODEL, gpt4o_mini_usage["input_tokens"], gpt4o_mini_usage["output_tokens"])
    gemini_cost = calculate_cost(GEMINI_MODEL, gemini_usage["input_tokens"], gemini_usage["output_tokens"])
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Keep history limited to last 3 turns (6 messages)
        if len(history) > 6:
            history = history[-6:]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        ...
        for model_key, model_data in result.items():
            ...
            truncated_response = (response_text[:50] + "...") if len(response_text) > 50 else response_text
            ...
            table += f"| {prompt} | {model_name} | {truncated_response} | {latency:.2f}s | {input_tokens}/{output_tokens} | ${cost:.6f} |\n"
